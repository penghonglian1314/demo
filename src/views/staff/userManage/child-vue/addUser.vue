  ...
  <el-container class="dialog_main">
    <el-main>
      ...
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" :inline="true" status-icon class="demo-form-inline">
        <el-form-item label="帐号：" prop="UserID" style="width:45%" label-width="40%">
          <el-input v-model="ruleForm.UserID" style="width:100%" />
        </el-form-item>
        <el-form-item label="用户名：" prop="UserName" style="width:45%" label-width="40%">
          <el-input v-model="ruleForm.UserName" />
        </el-form-item>
        <el-form-item label="密码：" prop="Passwd" style="width:45%" label-width="40%">
        ...
        <el-form-item label="角色：" prop="UserRoles" style="width:45%" label-width="40%">
...
import { addProjectUser, addAccount } from '@/api/user'
export default {
  data() {
    return {
      ruleForm: {
  ...
  methods: {
    ...
    addUser() {
      addUser(this.ruleForm, this.addUserUrl).then(response => {
    ...
    addAccount() {
      ...
      params.Balance = 0
      addAccount(params, this.addAccountUrl).then(response => {
        // console.log(response)
        if (response.data.success) {
          this.$message({ message: '操作成功', type: 'success' })
          this.close()
