...
import { getFileServerToken } from '@/api/upload'
...
export default {
    ...
    if (FtpType) {
      ...
      if (FtpType === '1') {
        ...
        if (postData && setting) {
          ...
          // 本地token未过期
          if (JSON.parse(window.atob(this.postData.token.split(':')[2])).deadline - 800 > Math.floor((new Date().getTime()) / 1000)) {
      ...
      // 腾讯云
        ...
        if (postData && setting && JSON.parse(postData).expiredTime - 800 > Math.floor((new Date().getTime()) / 1000)) {
          // console.log('本地')
          this.postData = JSON.parse(postData)
          this.setting = JSON.parse(setting)
          this.which = 'tx'
  ...
  methods: {
    async queryInit() {
      ...
      const res = JSON.parse((await getFileServerToken()).data.data)
      if (res.FtpType === 1) {
    ...
    fileChange(filelist) {
      ...
      this.$emit('fileChange', filelist.map(e => e.url))
    },
    submitFile(file, key) {
